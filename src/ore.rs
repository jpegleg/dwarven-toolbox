...
fn orecrypt<R: Read, W: Write>(input: &mut R, output: &mut W) -> io::Result<()> {
    let mut rng = StdRng::from_entropy();
    let mut nonce = [0u8; 24];
    rng.fill(&mut nonce);
    let bnon = hex::encode(&nonce);
    let binding = "B000FFFF".to_owned() + &bnon;
...
fn unorecrypt<R: Read, W: Write>(input: &mut R, output: &mut W) -> io::Result<()> {
    let mut ciphertext = Vec::new();
...
fn main() -> io::Result<()> {
    ...
    if decrypt_flag {
        ...
        unorecrypt(&mut input_file, &mut output_file)?;
        ...
        let temp_file_path = format!("{}.tmp_{}", file_path, chaos);
        let mut output_file = File::create(&temp_file_path)?;
        orecrypt(&mut input_file, &mut output_file)?;
        std::fs::rename(&temp_file_path, file_path)?;
