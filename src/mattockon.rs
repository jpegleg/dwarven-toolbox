...
fn main() {
    ...
    if args.len() != 4 {
        println!("The first argument is the 12 byte hex nonce, the second is the message, and the third is the 32 byte hex key. If there is whitespace in the message input, surround in doublequotes.\n\nUsage example: mattockon 4450501658F5 \"The great forge awaits you!\" 09578F03D7F3FBEB0769A7A8AC35893 \n\nNote: don't use the example nonce and key, use newly generated ones and never repeat the same nonce! The \"anvil\" program can be used to generate a new nonce and key.");
        std::process::exit(1);
    }
